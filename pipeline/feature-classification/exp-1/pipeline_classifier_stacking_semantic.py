...
for idx_lopo_cv in range(len(id_patient_list)):
    ...
    for mod_data in range(len(data) - 1):
        ...
        rf_data_answer.append(np.concatenate((np.atleast_2d(
            pred_proba[:, pos_class_arg]).T, validation_data), axis=1))
    ...
    cgb = RandomForestClassifier(n_estimators=100, n_jobs=-1)
    ...
    for mod_data in range(len(data) - 1):
        ...
        pos_class_arg = np.ravel(np.argwhere(
            rf_ensemble[mod_data].classes_ == 1))[0]
        testing_inter.append(np.concatenate((np.atleast_2d(
            pred_proba[:, pos_class_arg]).T, testing_data), axis=1))
...
path_store = '/data/prostate/results/mp-mri-prostate/exp-1/stacking-semantic'
