...
def find_normalization_params(pat_adc, pat_gt, label):
    # Create the normalization object and load the model
    adc_norm = GaussianNormalization(ADCModality())
...
adc_norm_list = Parallel(n_jobs=48)(delayed(find_normalization_params)(
    p_adc,
    p_gt,
    label_gt)
                                   for p_adc, p_gt in
...
path_store = '/data/prostate/pre-processing/mp-mri-prostate/gaussian-adc'
