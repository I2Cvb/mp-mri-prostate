"""
This pipeline is intended to extract DCT features from T2W images.
"""
...
from protoclass.extraction import DCTExtraction
...
path_store = '/data/prostate/extraction/mp-mri-prostate/dct-t2w'
...
for id_p, (p_t2w, p_gt) in enumerate(zip(path_patients_list_t2w,
    ...
    # Create the Dct extractor
    dct_ext = DCTExtraction(t2w_mod)

    # Fit the data
    print 'The DCT statistics'
    dct_ext.fit(t2w_mod, ground_truth=gt_mod, cat=label_gt[0])

    # Extract the data
    print 'Extract the edge map'
    data = dct_ext.transform(t2w_mod, ground_truth=gt_mod, cat=label_gt[0])
