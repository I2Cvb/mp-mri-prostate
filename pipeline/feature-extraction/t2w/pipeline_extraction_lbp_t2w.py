"""
This pipeline is intended to extract LBP features from T2W images.
"""
...
path_store = '/data/prostate/extraction/mp-mri-prostate/lbp-t2w'
...
for id_p, (p_t2w, p_gt) in enumerate(zip(path_patients_list_t2w,
    ...
    for p, r in zip(p_arr, r_arr):
        ext = LBPExtraction(t2w_mod, p=p, r=r)
        ...
        pat_chg = (id_patient_list[id_p].lower().replace(' ', '_') +
                   '_lbp_' + str(p) + '_' + str(int(r)) + '_t2w.npy')
