"""
This pipeline is intended to extract DCT features from ADC images.
"""
...
path_store = '/data/prostate/extraction/mp-mri-prostate/dct-adc'
...
for id_p, (p_adc, p_gt) in enumerate(zip(path_patients_list_adc,
    ...
    dct_ext = DCTExtraction(adc_mod)

    # Fit the data
    print 'The DCT statistics'
    dct_ext.fit(adc_mod, ground_truth=gt_mod, cat=label_gt[0])

    # Extract the data
    print 'Extract the edge map'
    data = dct_ext.transform(adc_mod, ground_truth=gt_mod, cat=label_gt[0])
