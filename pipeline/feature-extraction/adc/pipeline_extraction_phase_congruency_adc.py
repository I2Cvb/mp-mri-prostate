...
path_store = '/data/prostate/extraction/mp-mri-prostate/phase-congruency-adc'
...
for id_p, (p_adc, p_gt) in enumerate(zip(path_patients_list_adc,
    ...
    adc_mod.update_histogram()

    # Create the Haralick extractor
    pc_ext = PhaseCongruencyExtraction(adc_mod, 'monogenic')

    # Fit the data
    print 'The Haralick statistics'
    pc_ext.fit(adc_mod, ground_truth=gt_mod, cat=label_gt[0])

    # Extract the data
    print 'Extract the edge map'
    data = pc_ext.transform(adc_mod, ground_truth=gt_mod, cat=label_gt[0])
