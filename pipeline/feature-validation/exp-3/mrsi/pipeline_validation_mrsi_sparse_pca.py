...
fresults = '/data/prostate/results/mp-mri-prostate/exp-3/mrsi/sparse-pca/results.pkl'
...
for idx_sp, sp in enumerate(sparsity_level):
    ...
    for idx_cv in range(len(testing_label_cv)):
        ...
        # Get the prediction
        pred_score = results[idx_sp][idx_cv][0]
        classes = results[idx_sp][idx_cv][1]
    ...
    ax.plot(mean_fpr, avg_tpr,
            label=r'$\lambda = {}$ - AUC $= {:1.3f} \pm {:1.3f}$'.format(
                sp, auc(mean_fpr, avg_tpr), np.std(auc_pat)),
            lw=2)
...
plt.title(r'MRSI classification with Sparse PCA decomposition with different $\lambda$')

handles, labels = ax.get_legend_handles_labels()
lgd = ax.legend(handles, labels, loc='lower right')#,
                #bbox_to_anchor=(1.4, 0.1))
# Save the plot
plt.savefig('results/exp-3/mrsi_sparse_pca.pdf',
