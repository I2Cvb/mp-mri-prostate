...
fresults = '/data/prostate/results/mp-mri-prostate/exp-3/t2w/select-model-percentile/results.pkl'
...
for idx_p, p in enumerate(percentiles):
    ...
    ax.plot(mean_fpr, avg_tpr,
            label=r'{} - AUC $= {:1.3f} \pm {:1.3f}$'.format(
...
plt.title(r'T2W classification using the a subset of features using feature significance')

handles, labels = ax.get_legend_handles_labels()
lgd = ax.legend(handles, labels, loc='lower right')#,
                #bbox_to_anchor=(1.4, 0.1))
# Save the plot
plt.savefig('results/exp-3/t2w_model.pdf',
